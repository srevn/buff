// Package api is buff's HTTP edge: a thin, stateless skin over a store.Store that speaks the
// /v1 content protocol and the unversioned /healthz probe. It is the only place a domain error
// becomes an HTTP status, and the only place the framing, trailer, and abort contract lives —
// everything below it is transport-agnostic.
//
// The server constructs no store; it takes the Store interface and relays opaque bytes through
// it, so a different storage medium is a wiring change elsewhere, never a change here. Three
// things make the relay correct under streaming. A read's framing is fixed once, at header
// time, from whether the target is finalized: a finalized clip is sent with an exact
// Content-Length, a live one chunked with a completion trailer — and the followable buffer
// returns a clean end only on a clean finalize, so a torn stream becomes an aborted connection
// and can never reach a client looking complete. A write is acknowledged only after its durable
// commit returns. And clip bytes only ever move through a pooled copy buffer, never onto the
// heap in bulk.
package api
...
type Options struct {
	...
	Version    string        // /healthz version string; empty means a built-in default
...
// Server is the HTTP handler for the content API. It holds the store it relays to, the routed
// mux, the same mux wrapped in the recover backstop, and the copy-buffer pool every streaming
// handler draws from. Construct one with New and use it as an http.Handler directly, or call
// HTTPServer for a tuned *http.Server.
type Server struct {
	store   store.Store
	opt     Options
	mux     *http.ServeMux
...
// routes, and wrapping them in the recover backstop. The copy-buffer pool yields pointers to
// fixed-size buffers, so returning one to the pool never allocates.
...
// routes registers the six handlers. HEAD has its own route, not a fallthrough to GET: Go maps
// HEAD to a GET handler when no HEAD route exists, and routing a metadata probe into the GET
// handler would claim a consume-once clip. The explicit HEAD route takes precedence and keeps
// the probe on the non-claiming Stat path. The list and health routes carry no name wildcard.
func (s *Server) routes() {
	...
	s.mux.HandleFunc("GET "+wire.PathHealthz, s.healthz)
}
...
// ReadTimeout and WriteTimeout are deliberately left zero — they would kill a long upload or a
// slow live follow — so a stall is bounded by the per-request idle deadlines and the request
// context instead, while only the header read, the keep-alive idle, and the header size are
// capped here. The lifecycle — BaseContext, ListenAndServe, Shutdown — is the caller's to wire.

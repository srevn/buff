...
//
// The followable buffer returns a clean io.EOF only when a generation finalized cleanly, so
// a nil io.CopyBuffer error is the one and only success. Any copy error is a torn stream —
// a truncated upload behind the buffer, an abort, a vanished or stalled client — and becomes
// panic(http.ErrAbortHandler), which Go turns into an abrupt connection close with no terminating
// chunk and no trailer, so a truncated read can never reach a client looking complete. The
// framing is chosen once, from the c.Finalized snapshot taken under the store handle lock at Open:
// finalized is terminal, and a live→finalized transition is absorbed by the follower reaching a
// clean EOF, so deciding once never goes stale.
func (s *Server) stream(w http.ResponseWriter, src io.Reader, c clip.Clip) {
	...
	if c.Finalized {
		// Load-bearing, not redundant. Without it Go buffers the body in its fixed response buffer (2048
		// bytes) and, the instant a copy write overflows that buffer while the handler is still running,
		// commits the response to chunked transfer. This arm declares no trailer, so a wholly-delivered
		// clip larger than 2048 bytes would then arrive chunked with no completion proof and every client
		// would read it as torn. Declaring the exact length forces fixed-length framing at every size —
		// the finalized arm's completion proof, and what keeps a finalized read plain length-delimited
		// bytes for any third-party client.
		...
		// announced before the body. A live clip's length is unknown at header time, so fixed-
		// length framing is impossible; chunked plus a trailer is the only framing that can prove the
	...
	if live {
		// Flush the status and metadata now, before the first byte. A follower blocks for bytes that may
		// not arrive for a while, so without this an attach to a clip still being written would withhold
		// the whole response — even its metadata — until the producer happens to write, and a consumer
		// could not tell "attached, waiting" from "still connecting". Flushing on attach makes the live
		// response observable at once, the bytes following as they are produced. A flush failure means
		// the client is already gone: treat it as the torn stream a failed first write would, so the
		// abort contract is identical either way.

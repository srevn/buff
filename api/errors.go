...
// errBadRequest is the api-internal sentinel for a malformed request the store never sees: an
// unknown clip kind, a TTL that is not a non-negative duration, a filename that will not
// percent-decode. It maps to the generic bad_request row. A bad filename keeps its own
...
// wrapped store error still resolves to the right row and the status and sentinel come from the
// canonical table rather than being typed by hand. An error matching no row is an unexpected
// internal fault.
var errMap = []struct {
	err  error
	info wire.ErrInfo
...
// writeErr sends an error response before any body has started: the Buff-Error sentinel header,
// the status, and a one-line body that is the sentinel itself — never the underlying cause,
// which may carry a clip name or an internal detail. The cause of a genuine internal failure
// is logged here instead, the one piece a constant body omits that an operator needs; every
// other status is self-describing, so its cause is not logged here; the per-request access
// log records the rest. A HEAD response carries the header and status but no
// body. This is for the pre-stream path only: once a body has started, an error is a panic, not
// a status that can no longer be changed.

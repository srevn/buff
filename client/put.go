...
// the cap exists to stop, so it cannot be closed server-side.
//
// A failed round-trip with no response is not always the network's fault: the body the caller
// handed in may have faulted under the transport's own read. The client tells the two apart by
// watching that body (recordingReader) — a source-read failure becomes ErrSource, a genuine
...
// the connection write while the source read keeps succeeding, so the recorder stays empty and
// the status (or, in the narrow window, ErrUnreachable) still wins.
//
...
func (c *Client) Put(ctx context.Context, name string, r io.Reader, m clip.Meta, o PutOpts) (clip.Clip, error) {
	...
	// Watch the body as the transport reads it, so a body-read failure can be told from a
	// connection failure: do collapses both into one ErrUnreachable, but only the former means
	// the caller's source faulted.
	rec := &recordingReader{r: r}
	resp, err := c.do(ctx, http.MethodPut, c.clipURL(name), rec, encodeHeaders(m, o))
	if err != nil {
		// do reports every failed round-trip as ErrUnreachable, but if the body itself faulted it
		// was the source, not the server. Prefer the recorded read error, which becomes ErrSource
		// and supersedes do's ErrUnreachable entirely so the message names the real cause — except
		// a cancellation, which is the operation being stopped rather than the source failing and
		// stays the transport's report, leaving the existing cancel→ErrUnreachable path (normalised
		// to 130 at the process boundary) unchanged.
...
// round-trip can be attributed to the source rather than the network. net/http reads this body on
// its own write-loop goroutine, so err is written there and read in Put — yet without a lock,
// because the read happens only after do returns an error, and net/http guarantees the body is
// finished by then: its transport waits for the write loop to terminate before returning any
// round-trip error (mapRoundTripError's "wait ... to avoid data races on callers who mutate the
// request on failure"), so the goroutine's last write to err happens-before do returns happens-
// before the read. The early-response paths — a 2xx, or a cap's 4xx/5xx — return a nil error, and
// there the field is never read. One read, strictly after the last write: race-free by net/http's
// own contract, not by chance. The EOF skip mirrors io.Copy's, the loop that drives this Read: a
// clean end is not a fault.

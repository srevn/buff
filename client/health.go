...
// Features lists the capabilities this build implements, advertised for operator diagnosis and
// introspection; buff itself never gates on it — client and server ship as one binary — so it is a
// readout of what the server supports, not a contract a caller must check before an operation.
type Health struct {

...
// but left out of Rows, which would silently relocate the very gap Rows exists to close. This test
// shuts that door the way purity_test.go shuts the import door: by reading the package's own source.
// It parses every production file, collects the names of all package-level ErrInfo vars and the
// identifiers listed in the []ErrInfo Rows literal, and asserts the two sets are equal. So "add a
// var, forget Rows" is a build failure here, which forces the value pin (wire_test.go) and then the
// map coverage (api/, client/) — every link in the drift chain made unforgettable, not just unlikely.
func TestRowsEnumeratesDeclaredVars(t *testing.T) {
	...
	// contract source, never itself. The package dir is the working directory under go test, the
	// same assumption purity_test.go makes.
	...
	for _, file := range pkg.GoFiles {
		...
		for _, decl := range f.Decls {
			...
			for _, spec := range gen.Specs {
				...
				// An explicit `var ErrX ErrInfo [= ...]` names ErrInfo rows up front, whatever the
				// value form; the untyped `var ErrX = ErrInfo{...}` form is caught by its value below.

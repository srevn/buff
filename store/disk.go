...
// The on-disk layout vocabulary. Every path the disk medium builds is composed from these, so the
// shape of the tree — and the names a later recovery pass will look for — is defined in one place:
//
//	clips/<genid>/
...
// directly under one flat clips/. The caller's name appears in no path: it lives only in meta.json
// and the RAM registry. Two consequences follow, both load-bearing. Traversal is impossible by
// construction, and no future name shape — Unicode, hierarchical — can change the layout, because
// the name is never a path component. And a generation is one whole directory created, published,
// claimed, and reclaimed as a unit, with no per-name parent left behind once its last generation is
// gone — so high-cardinality, never-reused names leave nothing behind.
//
...
// It is stateless per generation — every path is derived from the generation id on demand — so it
// holds only the root it writes through and whether to flush each durable step.
//
// fsync and checksum are parameters of the medium, not fields of Config. Config is medium-agnostic
// policy whose zero value disables each knob; durability must instead default on, and both flushing
// and checksumming are meaningful only on disk. Making them construction arguments of the one
// constructor that has a disk keeps Config pure and makes them explicit choices where the disk is
// chosen. The logger is injected for the same reason — no global mutable state — and recovery is
// its one user: it logs loudly when it quarantines, and a boot summary when it had work to do.
...
// whether to store and verify a content checksum, and the logger recovery reports through. The zero
// value is the safe-but-fast choice for tests and embedding — no flushing, no checksum, the default
// logger; a server maps its environment into it. It is a struct rather than positional booleans so
// each knob reads at the call site and so a future disk knob is an additive field, not a signature
// change.
...
// classifies every generation directory — keeping survivors, reclaiming garbage, quarantining
// the uninterpretable — and restore rebuilds the in-memory registry, the per-name id seed, and
// the quota from exactly what survived. The error return reports a construction failure the store
...
// than lazily means no per-name or recovery path ever has to create a shared parent, so two first-
// ever operations can never race over whether a parent's entry in the data root is durable: by the
// time any generation is written or quarantined, both parents are already durable. One directory
// sync of the data root makes both new entries durable at once.
...
// is what lets every generation share one flat clips/ with no name-derived parent: a generation is
// created, published, claimed, and reclaimed as one whole directory keyed by its id.
...
// removes this generation's own directory, so a half-made generation never lingers; it touches
// no sibling, so a prior finalized generation is never at risk. It runs under the per-name handle
// lock, so it stays bounded: one directory creation, the data file, and the single directory fsync
// that makes the new entry durable.
...
// mk creates one directory, reporting whether it was the creator. An already-existing directory is
// not an error — a restart over a populated data root finds clips/ and quarantine/ already there
// — but every other failure is returned. The created flag lets ensureDirs fsync the data root only
// when it actually added an entry.
...
func (m *diskMedium) finalize(g *generation) (committed bool, err error) {
	...
	// With checksums on, hash the data into the record before publishing it. The writer has already
	// synced the bytes, so this re-read is page-cache-hot and streamed — no clip bytes land on the
	// heap — and recovery verifies the same hash the same way. Off by default: the trade is doubling
	// finalize's read IO, declined unless asked for.
	if m.checksum {
...
// when durability is on so the record's data is durable before the commit makes its name durable.
// Marshalling happens before the file is touched, so a marshal failure leaves no stray tmp; the
// file is closed on every path. The tmp bears the final name only after the caller's commit, so a
// half-written record is never mistaken for a finalized one.
...
// resolve as a readable current, so a racing reader finds nothing to claim, and — made durable by
// the directory fsync inside commit — it ensures a crash cannot resurrect a secret already handed
// out. The store calls it under the handle lock, the window in which exactly one reader wins the
// claim, before any byte ships.
//
// It forwards commit's committed flag: the rename is the irreversible step. If the rename fails
// the claim never took and the store reverts it; if the rename succeeds but its fsync does not, the
// claim took but is not durable — meta.json is already gone — and the store destroys the generation
...
// delete must never become its error — hence no return value.
//
...
// be invisible until the next boot. The bytes stay on disk until something reclaims them, and
// the medium does not promise when — the path that triggered the reclaim decides whether the next
// startup treats the leftover as garbage to GC or as a finalized survivor to reinstate. A consume-
// once clip's plaintext is the case that earns a distinct, greppable line: a deliver-once secret
// kept past its one delivery is the lingering byte an operator most needs to see.
...
// does. A caller that must undo a failed commit can revert only when committed is false; once it
// is true the rename has happened and only forward cleanup is correct. committed is false only when
...
// within it — a renamed-in file, a freshly created subdirectory — durable. On darwin the Go runtime
// issues a full-device flush inside Sync, falling back automatically on filesystems that reject it,
// so this is the genuine durability primitive on every supported platform with no platform-specific
// code of buff's own.
...
// crc32cTable is the Castagnoli polynomial table the content checksum hashes against, computed once
// and shared. It is immutable lookup data, not mutable global state — Castagnoli is chosen because
// its CRC is hardware-accelerated on common CPUs.
...
// The algorithm prefix names the hash, leaving room for a future one to be added without ambiguity;
// under this schema version only "crc32c:" is ever written.

...
// close-and-replace notify channel that lock guards, and the three — and only three — ways that
// lock is ever taken. peek reads under it, transition mutates under it and wakes, await parks under
// it until the name becomes readable and then commits under the same hold. Promoting the lock into
// a type with exactly these roles is what makes the wake structural: wakeLocked is gate-private,
// fired only by transition and await, so a readable-state change cannot escape without a wake
// and nothing outside the gate can fire a stray one. The hand-written "remember to wake" that was
// smeared across every mutator becomes one owner with one job.
//
// The gate is content-agnostic — it names no generation and knows nothing of the resolution rule.
// The state it coordinates (current, live, the id seed) lives on the handle and is touched only
// inside the closures peek/transition/await run under the lock — save once at single-threaded
// startup, where restore seeds a recovered name's fields directly, with no goroutine yet serving to
// contend with or to wake. That agnosticism is deliberate: it lets the gate be proven in isolation
// against a fake predicate exactly as the followable buffer is proven against a fake backing, and
// it is why await is a free function generic over the resolved value rather than a method. The
// cost of guarding foreign fields is that one discipline cannot be made structural — a closure must
// touch only handle state, leaf-lock quota ops, and the bounded medium IO that already ran under
// this lock, never registry.mu (which sits above the gate in the lock hierarchy) nor a re-entry
// of the gate itself. Those two fail unalike, and the dangerous one is quiet: a re-entered gate
// self-deadlocks on the spot, every run, caught by any test that walks the path, while a misplaced
// registry.mu inverts the hierarchy into a deadlock that surfaces only under contention — so it is
// the residue a new mutator's author must consciously guard, the loud one advertising itself. The
// wake, the part every mutator used to get wrong, now cannot be.
//
...
// exactly one of peek/transition/await and mu is taken nowhere else.
...
// transition runs fn under the lock and wakes iff fn reports the readable state moved — the mutate-
// and-wake role. fn returns true when it changed what a read resolves (an install, a finalize flip,
// a cleared pointer) and false when it touched nothing observable (a rejected admission, a guard
// whose target was already gone, a medium rename that never took). The gate owns the wake, so a
// mutator cannot forget it and cannot fire a stray one; it can only declare the bool, which is
// true on every genuine change and false only on a provable no-op — and the dangerous direction,
// returning false when state changed, is the unnatural one to write. Anything fn must do off the
// lock — reclaim a superseded home, release a lease, destroy a consumed generation — it captures
// into the caller's locals for the caller to run after transition returns; see the gate doc for the
// one convention a closure must keep.

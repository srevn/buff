...
// handle is created and then kept (current is not nil, so release does not evict it) with no new
// registry API. The quota is set last, absolute, to exactly what survived.

...
// holding that name's generations and the lock that serializes work on them.
//
...
// no generation. The emptiness check and the map delete both run under a nested handle.mu, taken
// inside registry.mu in the canonical order, so no window exists between deciding the handle is
// empty and removing it.
//
// Why the read of current and live is race-free is load-bearing, so state it exactly. Every mutator
// of those fields — Create, the Close flip, discard, Delete — holds a lease across the mutation
// and releases it only afterwards. So when this call drives the lease count to zero, no mutator is
// or can be in flight: mutual exclusion here comes from that lease invariant, not from handle.mu.
// registry.mu then supplies the visibility — the last mutator's write precedes its own release,
// which precedes this decrement — so the read sees the final values. The nested handle.mu is kept
// regardless of that argument: it makes the read locally, obviously correct, and stays safe even if
// some future mutator path were to touch the handle without first leasing it.

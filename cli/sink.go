...
// salvager is the capability a terminal gesture sink adds when it can rescue a consume-once delivery
// whose normal landing collided. Only the two sinks buff itself picks a name for — saveSink and
// newDirSink — implement it, because they are the only ones whose no-clobber landing can refuse a
// delivery the server has already spent; the -o sinks clobber a name the user spelled, so they never
// collide irrecoverably. salvage writes the whole, still-untouched body beside the sink's own
// colliding target under a sibling name made unique by the delivery's generation id, returning nil
// once the bytes are safely landed (and narrated) or an error that explains the loss. The sink
// contributes only the mechanism — land beside yourself — and never learns why: the consume-once
// knowledge stays in the flow (divertConsumeOnce), so one policy in one place covers every sink and a
// future terminal sink cannot silently re-forget the rescue.
type salvager interface {
	salvage(ctx context.Context, r io.Reader, m clip.Meta, gen string) error
}

// divertConsumeOnce is the paste flow's one rescue point for a consume-once delivery the server spent
// at Open and the chosen sink then could not land. The flow calls it only once it has established the
// delivery is recoverable — the clip is consume-once and the body is pristine (no byte consumed, no
// tear) — so the single question left is whether this refusal is a recoverable collision on a sink
// that knows how to land beside itself.
//
// Two filters, each necessary and neither sufficient: the sink must be a salvager (the -o sinks are
// not — the user named that destination, so its failure is theirs to see), and the refusal must be a
// collision rather than a permission or space fault an alternate name in the same directory would hit
// identically. A refusal that passes neither is returned unchanged, leaving the original error — and
// its exit code — to stand.
//
// The generation id is the sibling's uniqueness, and it is a wire value a foreign or buggy peer
// controls. An absent one cannot be trusted to form a distinct name, so rather than mint a
// degenerate, non-unique sibling (report.pdf. / slot-) the salvage refuses: the delivery is lost, but
// the loss is reported, never silent. A present-but-hostile id (separators, controls, over-long) is
// not guarded here — the sink's openInDir/ExtractNew re-validate the whole sibling name and reject it
// as a reported loss — so only an honest, usable id ever reaches a write.
...
// saveSink's O_EXCL open, and archive.ErrDestExists from newDirSink's ExtractNew. The merge-mode
// archive.ErrExists is deliberately absent: it arises only in the -o extractSink, never in a
...
// name Write could not claim — the archive half of saveSink.salvage, called by the flow under the
// same whole-body, recoverable-collision precondition. The sibling appends the delivery's generation
// id (project -> project-<gen>), unique per delivery, so the publish needs no probe and no loop: a
// single ExtractNew reads the body exactly once and renames the finished tree into place.
//
...
// succeeds and silently plants a bogus empty tree. One commit means a late race on this unique name
// (or a hostile, over-long gen that ExtractNew's singleComponent/confinement rejects) is the reported
// loss it is, never a silent empty directory. The narration is past tense and follows the publish, as
// narrateExtract's: ExtractNew is atomic, so reaching the line means the tree is wholly there.
...
// the working directory under its remembered filename, refusing to clobber an existing file. It is a
// pure disk-saver — its name is finally exact — touching the terminal only to narrate where the bytes
// landed.
//
...
// user never typed — is worth a line. Every disk-landing sink does the same (fileSink's directory arm
// for a file, newDirSink and extractSink for a tree); only an -o <file> the user spelled out in full,
// and bytes the terminal or a pipe already shows, stay silent.
//
// It is one of the two sinks that implement salvager. Its no-clobber save is the only file landing
// whose open can collide with a delivery the server has already spent: the server claims a
// consume-once delivery at Open, before any byte ships, so a refused save would lose the only copy.
// Rather than lose it, salvage lands the untouched body on a free sibling beside the colliding name
// (./report.<gen>.pdf). The decision to do so is the flow's, not the sink's (see divertConsumeOnce);
// saveSink only contributes the two mechanisms — save here, land-beside in salvage. fileSink's
// directory arm clobbers, so it never collides and never needs either.
...
// file copies always remember a basename and a text clip never reaches this sink. The normal save and
// the salvage land under names built from the same basename, so they share this one resolver.
...
// Write saves the clip's bytes to a no-clobber file in the working directory. A collision surfaces as
// os.ErrExist before any byte is read — openInDir's pre-commit boundary, where the chmod and the
...
// salvage lands a spent consume-once delivery on a free sibling beside the name Write could not claim
// — the flow calls it only when the body is still whole and the collision is recoverable. besideName
// splices the delivery's generation id in front of the extension (report.pdf -> report.<gen>.pdf), so
// the sibling is unique per delivery: a concurrent second salvage of the same name lands its own,
// distinct, sibling rather than racing this one. The open is still O_EXCL — the gen makes a real
// collision impossible for a buff peer, and on the astronomical chance of one, or a hostile or
// over-long gen openInDir's ValidFilename rejects, it fails closed to a reported loss rather than
// clobbering. The narration is present tense and precedes the non-atomic copy, exactly as
// narrateSave's: the copy can still tear after the line, so it promises the attempt and names where
// the bytes are going, not a finished file.
...
// narrateDivertedSave and narrateDivertedExtract are the consume-once salvage's landing notices — the
// diverted-landing counterparts of narrateSave and narrateExtract, and they keep the very same tense
// discipline, because diverting the landing does not change what each sink can promise. A salvaged
// file is a non-atomic copy that can tear after the line, so its notice is present tense and printed
// before the copy; a salvaged tree is an atomic ExtractNew already published, so its notice is past
// tense and printed after. Each names the collision that forced the diversion and the sibling the
// bytes went to, so even a torn file salvage still tells the user where the partial bytes are. (One
// shared neutral line would have to choose a single tense, mislabelling one sink — present tense lies
// about the published tree, past tense lies about the half-written file — so the two stay split, the
// same reason narrateSave and narrateExtract are two functions.)
...
// besideName forms the consume-once file salvage's sibling by splicing the delivery's generation id
// in front of name's extension: report.pdf -> report.<gen>.pdf, Makefile -> Makefile.<gen>,
// archive.tar.gz -> archive.tar.<gen>.gz. Keeping the extension last leaves the rescued file usable
// by an extension-aware tool without a rename — the one identity worth preserving on this rare path.
//
// name reached salvage having passed openInDir's ValidFilename (the no-clobber collision that brought
// us here is raised only after that check), so it is a known-good basename; splicing a hex run into
// it cannot introduce a separator or a control, leaving length the only way the result can be
// invalid — which the salvage's own openInDir catches as a reported loss. gen is a non-empty wire
// value the flow already vetted for presence; an otherwise-malformed gen is caught the same way, at
// the write, not here, so this stays a pure formatter.

...
// invocation is the fully resolved command: an action, its target slot, and the inputs that action
...
// belongs. It is its own type so the exit map routes every one of them to the generic usage exit
// and a caller can tell a grammar mistake from a network or domain failure. The message is the
// whole diagnostic, printed to stderr as-is.
...
// tokens is the lexical result of one scan over the arguments: the slots (each the text after an
// '@'), the bare paths in order, and the flag values. Classification here is purely syntactic — an
// '@' prefix, a leading '-', or neither — and never touches the filesystem.
type tokens struct {
	slots []string
	paths []string
	flags flags
...
// flags holds the whole fixed flag surface. Booleans default off; the value-bearing flags carry a
// "set" companion so an explicitly empty or zero value is distinguishable from an unset one.
type flags struct {
	copy, paste        bool // -c / -p
...
// flagSpec describes one flag: whether it consumes a following value, and the setter for its kind.
// Each flag has exactly one of setBool/setValue, matched to takesValue, so a value is never read
// into a boolean flag or vice versa.
...
// scan walks the arguments once, classifying each by syntax alone. "--" ends flag parsing, so a
// later argument that looks like a flag (a file named "-x") is taken as a path; everything after
// it is positional. A token starting with '@' is a slot — the non-empty text after the '@'. A token
// starting with '-' (other than a lone "-") is a flag, looked up in the fixed vocabulary, consuming
// the next argument as its value when it takes one and no "=value" was attached. Anything else —
// including a lone "-" — is a bare path. @-slots and paths may appear in any order; an unknown flag
// is a usage error, never a silent path.
...
// parseArgs runs the two front-end passes in order: the lexical scan that classifies each argument
// by syntax, then the grammar that resolves the scanned tokens into an invocation. It is the single
// front end Run drives; an error from either pass is the usage error the run reports.
...
// remainder is its name, which must not be empty — a bare "@" is a sigil with nothing to address,
// rejected here rather than sent to the server as an empty, confusing request.
...
// set applies one flag token to the flag set and reports how many following arguments it consumed
// (0 or 1). A "name=value" form supplies the value inline; otherwise a value-taking flag consumes
// the next argument, an error if there is none. A value attached to a boolean flag, or a flag not
// in the vocabulary, is a usage error.
...
// splitFlag splits a flag token at the first '=', so "--ttl=1h" and "-o=out" supply their value
// inline. With no '=' the value, if the flag takes one, comes from the next argument.
...
// order: at most one slot, then a single management action if a flag selects one, otherwise copy-
// vs-paste by mode detection. Every violation is a usage error naming the conflict; nothing here
// probes the filesystem.
func parse(t tokens, inIsTTY bool) (invocation, error) {
	f := t.flags

	// A help request short-circuits the grammar: -h/--help wins over a slot-count, management, or mode
	// error on the same line, so "buff -l -h" prints help rather than "conflicting actions" — a user
	// asking for help has by definition not got the line right yet, and surfacing help is the useful
	// answer. It does not override a *scan* error: an unknown flag earlier on the line ("buff --bogus
	// -h") still fails first, because scan runs before parse and a malformed flag is worth reporting.
	// Help answers offline, exactly like --version.
	if f.help {
...
// managementAction reports how many management flags are set and which action the last one names.
// The caller treats a count above one as a conflict and a count of one as that action; the returned
// action is meaningful only when the count is exactly one.
...
// parseManage resolves a management invocation and rejects the options that belong only to copy
// or paste. List and version take no slot or path; delete and stat each need exactly one slot and
// no path — a bare word with -d/-s is a path, which is the mistake of writing the slot without
// its '@'.
...
// mode resolves copy-vs-paste from whether path arguments are present, whether stdin is piped (not
// a TTY), and the -c/-p forces. The sigil already settled name-vs-path and the stream type settles
// direction, so there is no residual guess: a bare word at a TTY copies that file, never silently
// falling back to a paste. The forces exist for scripts where TTY detection is unreliable; -p
// ignores any incidental piped stdin, and -c with no source is an error rather than a guess.
...
// reslot re-attaches the '@' sigil to slot names for a diagnostic, so it shows them as the user
// wrote them.

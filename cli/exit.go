...
// name (a narrated beside-save), so the collision costs nothing. Only when the divert cannot form a
// unique sibling — a foreign peer that sent no generation id — does the collision stand as a 6, and
// then stderr reports the delivery lost rather than hiding it.
//
...
// read error wraps it), while an archive paste cancelled between entries, with no read in flight to
